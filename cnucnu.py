...
if __name__ == '__main__':
    ...
    parser.add_option("", "--dump-default-config", dest="yaml_file", help="dumps default config to ./cnucnu.yaml.sample", action="store_true", default=False)
    parser.add_option("", "--dry-run", dest="dry_run", help="Do not file or change bugs", default=False)

    (options, args) = parser.parse_args()
    if options.yaml_file:
        ofile = open("./cnucnu.yaml.sample", "wb")
        config = Config()
